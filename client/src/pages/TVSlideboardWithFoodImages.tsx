// TVSlideboardWithFoodImages.tsx
// Variant: same integrated logic (MOCK MODE, warp queue, supporters, background)
// Difference: Left menu shows ONLY 5 items and each has an image thumbnail.
// Tech: React + Tailwind (no external runtime deps). QR uses api.qrserver.com
...
/************************************
 * MOCK MODE & DUMMY DATA (from base)
 ***********************************/
...
function resolveMediaSource(raw?: string | null) {
  if (!raw) return null
 ...
 * Sections
 ***************************/
function FoodMenu5WithImages({ items }: { items: { name: string; price: number; tag?: string; image?: string | null }[] }) {
  const placeholder = 'https://picsum.photos/seed/food/256/256'
  return (
    <div className="h-full w-full rounded-2xl bg-black/35 backdrop-blur-sm border border-white/10 p-3 flex flex-col">
      <h2 className="text-xl md:text-2xl font-bold tracking-wide">เมนูแนะนำ (Top 5)</h2>
      <div className="mt-2 h-px bg-white/10" />
      <ul className="mt-3 space-y-3 overflow-y-auto pr-2 custom-scroll">
        {items.slice(0, 5).map((it, i) => {
          const img = resolveMediaSource(it.image || '') || placeholder
          return (
            <li key={i} className="flex items-center gap-3">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={img} alt={it.name} className="h-16 w-16 md:h-20 md:w-20 rounded-xl object-cover ring-2 ring-white/20 border border-white/20" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <div className="truncate">
                    <span className="text-base md:text-lg font-semibold">{it.name}</span>
                    {it.tag && <span className="ml-2 text-[10px] md:text-xs rounded bg-white/10 px-2 py-0.5">{it.tag}</span>}
                  </div>
                  <span className="text-base md:text-lg font-bold tabular-nums">฿{it.price.toFixed(0)}</span>
                </div>
              </div>
            </li>
          )
 ...
 * Main Screen
 ***************************/
export default function TVSlideboardWithFoodImages() {
  ...
  const bgImageUrl = useMemo(() => {
    ...
    return imageUrl
  }, [settings?.backgroundImage, extractImageColors])
  ...
  // 5 menu items with images (replace URLs with your assets later)
  const foodItems = useMemo(() => [
    { name: 'ปีกไก่ทอดเกลือ', price: 129, tag: 'Best', image: 'https://picsum.photos/seed/wings/320/240' },
    { name: 'เฟรนช์ฟรายส์ชีส', price: 129, image: 'https://picsum.photos/seed/fries/320/240' },
    { name: 'ยำหมูยอไข่แดง', price: 159, tag: 'Spicy', image: 'https://picsum.photos/seed/yum/320/240' },
    { name: 'คอหมูย่างจิ้มแจ่ว', price: 149, image: 'https://picsum.photos/seed/porkneck/320/240' },
    { name: 'ข้าวผัดหมู', price: 79, image: 'https://picsum.photos/seed/friedrice/320/240' },
  ], [])
    ...
    >
      ...
      <div className="h-full p-4">
        <GridLayout rows={6} cols={8} gap={12} showGrid={false} showIndex={false} style={{ height: '100%' }}>
          {/* Left rail: Food menu with images (2 cols, full height) */}
          <Box startRow={1} startCol={1} rowSpan={6} colSpan={2}>
            <FoodMenu5WithImages items={foodItems} />
          </Box>
